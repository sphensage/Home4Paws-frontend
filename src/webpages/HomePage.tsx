...
const HomePage = () => {
  ...
  const handleNext = () => { if (currentPage < lastPage) fetchPosts(currentPage + 1); };
  const handlePrev = () => { if (currentPage > 1) fetchPosts(currentPage - 1); };
    ...
    <>
      ...
      >
        ...
        <p className="mt-2" style={{ fontSize: "1.5rem" }}>
          A forum for sharing pet adoption and re-homing posts to support animal welfare.
        </p>
        <button
      ...
      {showCreateModal && (
        ...
        >
          <div
            ...
            style={{ background: "white", borderRadius: 8, padding: 20, width: "min(720px, 95%)", maxHeight: "85vh", overflow: "auto" }}
          >
      ...
      {showPostItemModal && activePaw && (
        <div
          ...
          style={{ position: "fixed", inset: 0, backgroundColor: "rgba(0,0,0,0.5)", display: "flex", justifyContent: "center", alignItems: "center", zIndex: 9999 }}
        >
          <div
            onClick={(e) => e.stopPropagation()}
            style={{ background: "white", borderRadius: 8, padding: 20, width: "min(720px, 95%)", maxHeight: "85vh", overflow: "auto", color: "#8b2e58" }}
          >
